  ...
  : ADMaterial<compute_stage>(parameters)
{
  ...
  for (unsigned int t = 0; t < _t_points.size(); ++t)
  {
    ...
    _rotation_matrix[t] =
        &getMaterialProperty<RankTwoTensor>("rotation_matrix_t_points_" + std::to_string(t));
    _global_stress[t] =
  ...
  for (unsigned int i = 0; i < _t_points.size(); ++i)
  {
    ...
    for (unsigned int ii = 0; ii < 3; ++ii)
      for (unsigned int jj = 0; jj < 3; ++jj)
         temp(ii, jj) = MetaPhysicL::raw_value((*_stress[i])[_qp](ii, jj));
    (*_global_stress[i])[_qp] = (*_rotation_matrix[i])[_qp].transpose() * temp * (*_rotation_matrix[i])[_qp];
  }
}
