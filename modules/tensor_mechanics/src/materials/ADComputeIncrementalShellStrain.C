      ...
      : ADMaterial<compute_stage>(parameters),
        ...
        _sol_old(_nonlinear_sys.solutionOld())
        {
          ...
          for (unsigned int i = 0; i < _t_points.size(); ++i)
          {
             ...
             _dxyz_dxi_old[i] = &getMaterialPropertyOldByName<RealVectorValue>("dxyz_dxi_t_points_" + std::to_string(i));
             _dxyz_deta[i] = &declareADProperty<RealVectorValue>("dxyz_deta_t_points_" + std::to_string(i));
             _dxyz_deta_old[i] = &getMaterialPropertyOldByName<RealVectorValue>("dxyz_deta_t_points_" + std::to_string(i));
  ...
  for (unsigned int i = 0; i < _2d_points.size(); ++i)
  {
    for (unsigned int j = 0; j < _t_points.size(); ++j)
    {
      ...
      (*_B[j])[i].zero();
        ...
        _g1_A = 0.5 * ((*_nodes[2]) - (*_nodes[3])) + _t_points[j](0)/4.0 * _thickness[i] * (_node_normal[2] - _node_normal[3]);
        _g1_C = 0.5 * ((*_nodes[1]) - (*_nodes[0])) + _t_points[j](0)/4.0 * _thickness[i] * (_node_normal[1] - _node_normal[0]);
        _g2_B = 0.5 * ((*_nodes[3]) - (*_nodes[0])) + _t_points[j](0)/4.0 * _thickness[i] * (_node_normal[3] - _node_normal[0]);
        _g2_D = 0.5 * ((*_nodes[2]) - (*_nodes[1])) + _t_points[j](0)/4.0 * _thickness[i] * (_node_normal[2] - _node_normal[1]);
...
ADComputeIncrementalShellStrain<RESIDUAL>::computeSolnVector()
{
  ...
  for (unsigned int j = 0; j < 4; ++j)
  {
    ...
    for (unsigned int i = 0; i < _ndisp; ++i)
    {
      ...
      _soln_vector(j+i*_nodes.size()) = (*_sol)(_soln_disp_index[j][i]) - _sol_old(_soln_disp_index[j][i]);
    }

    for (unsigned int i = 0; i < _nrot; ++i)
    {
      _soln_rot_index[j][i] = _nodes[j]->dof_number(_nonlinear_sys.number(), _rot_num[i], 0);
      _soln_vector(j+12+i*_nodes.size()) = (*_sol)(_soln_rot_index[j][i]) - _sol_old(_soln_rot_index[j][i]);
    }
  }
}
