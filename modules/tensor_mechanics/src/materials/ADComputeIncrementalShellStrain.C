...
ADComputeIncrementalShellStrain<compute_stage>::ADComputeIncrementalShellStrain(const InputParameters & parameters)
      : ADMaterial<compute_stage>(parameters),
        ...
        _Jmap()
        {
          ...
          {
             _strain_increment[i] = &declareADProperty<RankTwoTensor>("strain_increment_t_points_" + std::to_string(i));
             _total_strain[i] = &declareADProperty<RankTwoTensor>("total_strain_t_points_" + std::to_string(i));
...
ADComputeIncrementalShellStrain<compute_stage>::initialSetup()
{
  ...
  {
    _dxyz_dxi[t].resize(_2d_points.size());
    _dxyz_deta[t].resize(_2d_points.size());
    _dxyz_dzeta[t].resize(_2d_points.size());

    for (unsigned int qp = 0; qp < _2d_points.size(); ++qp)
    {
      _dxyz_dxi[t][qp] = c;
      _dxyz_deta[t][qp] = c;
      _dxyz_dzeta[t][qp] = c;
...
ADComputeIncrementalShellStrain<compute_stage>::computeProperties()
{
  ...
  for (unsigned int i = 0; i < _2d_points.size(); ++i)
  {
    for (unsigned int j = 0; j < _t_points.size(); ++j)
    {
      ...
      for (unsigned int component = 0; component < 3; ++component)
      {
        ...
        {
          _dxyz_dxi[j][i](component) += _dphidxi_map[k][i] * ((*_nodes[k])(component)) + _t_points[j](0) / 2.0 * _thickness[i] * _dphidxi_map[k][i] * en(component);
          _dxyz_deta[j][i](component) += _dphideta_map[k][i] * ((*_nodes[k])(component)) + _t_points[j](0) / 2.0 * _thickness[i] * _dphideta_map[k][i] * en(component);
  ...
  for (unsigned int j = 0; j < _nodes.size(); ++j)
  {
    ...
    for (unsigned int i = 0; i < _ndisp; ++i)
    {
      ...
      _soln_vector(j+i*_nodes.size()) = sol(_soln_disp_index[j][i]) - sol_old(_soln_disp_index[j][i]);
    }

    for (unsigned int i = 0; i < _nrot; ++i)
    {
      _soln_rot_index[j][i] = _nodes[j]->dof_number(_nonlinear_sys.number(), _rot_num[i], 0);
      _soln_vector(j+12+i*_nodes.size()) = sol(_soln_rot_index[j][i]) - sol_old(_soln_rot_index[j][i]);
    }
  }

  printf("_soln_vector:%e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e, %e\n", MetaPhysicL::raw_value(_soln_vector(0)), MetaPhysicL::raw_value(_soln_vector(1)), MetaPhysicL::raw_value(_soln_vector(2)), MetaPhysicL::raw_value(_soln_vector(3)), MetaPhysicL::raw_value(_soln_vector(4)), MetaPhysicL::raw_value(_soln_vector(5)), MetaPhysicL::raw_value(_soln_vector(6)), MetaPhysicL::raw_value(_soln_vector(7)),
MetaPhysicL::raw_value(_soln_vector(8)), MetaPhysicL::raw_value(_soln_vector(9)), MetaPhysicL::raw_value(_soln_vector(10)), MetaPhysicL::raw_value(_soln_vector(11)), MetaPhysicL::raw_value(_soln_vector(12)),
MetaPhysicL::raw_value(_soln_vector(13)),MetaPhysicL::raw_value(_soln_vector(14)), MetaPhysicL::raw_value(_soln_vector(15)), MetaPhysicL::raw_value(_soln_vector(16)), MetaPhysicL::raw_value(_soln_vector(17)), MetaPhysicL::raw_value(_soln_vector(18)), MetaPhysicL::raw_value(_soln_vector(19)));
  // compute nodal local axis
  ADRealVectorValue x2;
  x2(1) = 1;
  ADRealVectorValue x3;
  x3(2) = 1;
  ...
  for (unsigned int k = 0; k < _nodes.size(); ++k)
  {
    ...
    if (_large_strain)
    {
      ...
      printf("node_normal: %e, %e, %e \n", MetaPhysicL::raw_value(_node_normal[k](0)), MetaPhysicL::raw_value(_node_normal[k](1)), MetaPhysicL::raw_value(_node_normal[k](2)));
    }
    else
  ...
  for (unsigned int i = 0; i < _2d_points.size(); ++i)
  {
    for (unsigned int j = 0; j < _t_points.size(); ++j)
    {
      for (unsigned int component = 0; component < 3; ++component)
      {
        ...
        for (unsigned int k = 0; k < _nodes.size(); ++k)
        {
          _dxyz_dxi[j][i](component) += _dphidxi_map[k][i] * ((*_nodes[k])(component) + sol_old(_soln_disp_index[k][component]) ) + _t_points[j](0) / 2.0 * _thickness[i] * _dphidxi_map[k][i] * _node_normal_old[k](component);
          _dxyz_deta[j][i](component) += _dphideta_map[k][i] * ((*_nodes[k])(component) + sol_old(_soln_disp_index[k][component]) ) + _t_points[j](0) / 2.0 * _thickness[i] * _dphideta_map[k][i] * _node_normal_old[k](component);
          _dxyz_dzeta[j][i](component) += _thickness[i] * _phi_map[k][i] * _node_normal_old[k](component) / 2.0;
        }
      }
    }
  }
  ...
  for (unsigned int i = 0; i < _2d_points.size(); ++i)
  {
    for (unsigned int j = 0; j < _t_points.size(); ++j)
    {
      ...
      (*_B[j])[i].zero();
        ...
        ADRealVectorValue g3_A = _thickness[i] / 4.0 * (_node_normal_old[2] + _node_normal_old[3]);
        ADRealVectorValue g3_C = _thickness[i] / 4.0 * (_node_normal_old[0] + _node_normal_old[1]);
        ADRealVectorValue g3_B = _thickness[i] / 4.0 * (_node_normal_old[0] + _node_normal_old[3]);
        ADRealVectorValue g3_D = _thickness[i] / 4.0 * (_node_normal_old[1] + _node_normal_old[2]);

        ADRealVectorValue g1_A = 0.5 * ((*_nodes[2]) - (*_nodes[3])) + _t_points[j](0)/4.0 * _thickness[i] * (_node_normal_old[2] - _node_normal_old[3]);
        ADRealVectorValue g1_C = 0.5 * ((*_nodes[1]) - (*_nodes[0])) + _t_points[j](0)/4.0 * _thickness[i] * (_node_normal_old[1] - _node_normal_old[0]);
        ADRealVectorValue g2_B = 0.5 * ((*_nodes[3]) - (*_nodes[0])) + _t_points[j](0)/4.0 * _thickness[i] * (_node_normal_old[3] - _node_normal_old[0]);
        ADRealVectorValue g2_D = 0.5 * ((*_nodes[2]) - (*_nodes[1])) + _t_points[j](0)/4.0 * _thickness[i] * (_node_normal_old[2] - _node_normal_old[1]);
        ...
        if (_large_strain)
      {
        ...
        for (unsigned int component = 0; component < 3; component ++)
        {
          (*_B[j])[i](3,2+ component * 4) = 1.0/8.0 * (1.0 + _2d_points[i](1)) * g3_A(component);
          (*_B[j])[i](3,3+ component * 4) = 1.0/8.0 * (1.0 + _2d_points[i](1)) * -g3_A(component);
          (*_B[j])[i](3,1+ component * 4) = 1.0/8.0 * (1.0 - _2d_points[i](1)) * g3_C(component);
          (*_B[j])[i](3,component * 4) = 1.0/8.0 * (1.0 - _2d_points[i](1)) * -g3_C(component);
        }
        (*_B[j])[i](3, 14) = 1.0/8.0 * (1.0 + _2d_points[i](1)) * 0.5 * _thickness[i] * g1_A * -_V2[2];
        (*_B[j])[i](3, 18) = 1.0/8.0 * (1.0 + _2d_points[i](1)) * 0.5 * _thickness[i] * g1_A * _V1[2];
        (*_B[j])[i](3, 15) = 1.0/8.0 * (1.0 + _2d_points[i](1)) * 0.5 * _thickness[i] * g1_A * -_V2[3];
        (*_B[j])[i](3, 19) = 1.0/8.0 * (1.0 + _2d_points[i](1)) * 0.5 * _thickness[i] * g1_A * _V1[3];

        (*_B[j])[i](3, 13) = 1.0/8.0 * (1.0 - _2d_points[i](1)) * 0.5 * _thickness[i] * g1_C * -_V2[1];
        (*_B[j])[i](3, 17) = 1.0/8.0 * (1.0 - _2d_points[i](1)) * 0.5 * _thickness[i] * g1_C * _V1[1];
        (*_B[j])[i](3, 12) = 1.0/8.0 * (1.0 - _2d_points[i](1)) * 0.5 * _thickness[i] * g1_C * -_V2[0];
        (*_B[j])[i](3, 16) = 1.0/8.0 * (1.0 - _2d_points[i](1)) * 0.5 * _thickness[i] * g1_C * _V1[0];

        // corresponding to strain(1,2)
        for (unsigned int component = 0; component < 3; component ++)
        {
          (*_B[j])[i](4,2+ component * 4) = 1.0/8.0 * (1.0 + _2d_points[i](0)) * g3_D(component);
          (*_B[j])[i](4,1+ component * 4) = 1.0/8.0 * (1.0 + _2d_points[i](0)) * -g3_D(component);
          (*_B[j])[i](4,3+ component * 4) = 1.0/8.0 * (1.0 - _2d_points[i](0)) * g3_B(component);
          (*_B[j])[i](4,component * 4) = 1.0/8.0 * (1.0 - _2d_points[i](0)) * -g3_B(component);
        }
        (*_B[j])[i](4, 14) = 1.0/8.0 * (1.0 + _2d_points[i](0)) * 0.5 * _thickness[i] * g2_D * -_V2[2];
        (*_B[j])[i](4, 18) = 1.0/8.0 * (1.0 + _2d_points[i](0)) * 0.5 * _thickness[i] * g2_D * _V1[2];
        (*_B[j])[i](4, 13) = 1.0/8.0 * (1.0 + _2d_points[i](0)) * 0.5 * _thickness[i] * g2_D * -_V2[1];
        (*_B[j])[i](4, 17) = 1.0/8.0 * (1.0 + _2d_points[i](0)) * 0.5 * _thickness[i] * g2_D * _V1[1];

        (*_B[j])[i](4, 15) = 1.0/8.0 * (1.0 - _2d_points[i](0)) * 0.5 * _thickness[i] * g2_B * -_V2[3];
        (*_B[j])[i](4, 19) = 1.0/8.0 * (1.0 - _2d_points[i](0)) * 0.5 * _thickness[i] * g2_B * _V1[3];
        (*_B[j])[i](4, 12) = 1.0/8.0 * (1.0 - _2d_points[i](0)) * 0.5 * _thickness[i] * g2_B * -_V2[0];
        (*_B[j])[i](4, 16) = 1.0/8.0 * (1.0 - _2d_points[i](0)) * 0.5 * _thickness[i] * g2_B * _V1[0];
...
template <ComputeStage compute_stage>
void
ADComputeIncrementalShellStrain<compute_stage>::computeLargeStrain()
{
