...
ADComputeFiniteShellStrain<compute_stage>::ADComputeFiniteShellStrain(const InputParameters & parameters)
      : ADComputeIncrementalShellStrain<compute_stage>(parameters),
        _BNL()
{
  ...
  for (unsigned int i = 0; i < _2d_points.size(); ++i)
  {
    for (unsigned int j = 0; j < _t_points.size(); ++j)
    {
      for (unsigned int component = 0; component < 3; ++component)
      {
        ...
        for (unsigned int k = 0; k < _nodes.size(); ++k)
        {
          (*_dxyz_dxi[j])[i](component) += _dphidxi_map[k][i] * ((*_nodes[k])(component) + _sol_old(_soln_disp_index[k][component]) ) + _t_points[j](0) / 2.0 * _thickness[i] * _dphidxi_map[k][i] * _node_normal_old[k](component);
          (*_dxyz_deta[j])[i](component) += _dphideta_map[k][i] * ((*_nodes[k])(component) + _sol_old(_soln_disp_index[k][component]) ) + _t_points[j](0) / 2.0 * _thickness[i] * _dphideta_map[k][i] * _node_normal_old[k](component);
